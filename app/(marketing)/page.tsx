  ...
  return (
    <main>
      ...
      <section className="relative isolate overflow-hidden">
        <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(60%_60%_at_50%_0%,rgba(24,24,27,0.08),transparent_60%)]" />
        <div className="mx-auto max-w-6xl px-6 py-20 text-center">
          ...
          <div className="mt-8 flex items-center justify-center gap-3">
            <a className="rounded-md bg-zinc-900 px-5 py-3 text-white" href="/app">Open App</a>
            <a className="rounded-md border border-zinc-300 px-5 py-3" href="/signup">Get started</a>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-6 py-14">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {modules.map((m) => (
            <a key={m.title} href={m.href} className="group rounded-xl border bg-white p-5 shadow-sm transition hover:shadow">
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={p.name} className="rounded-xl border bg-white p-6 shadow-sm">
                ...
                <a href="/pricing" className="mt-5 inline-block rounded-md bg-zinc-900 px-4 py-2 text-white">Choose {p.name}</a>
              </div>
            ))}
          </div>
        </div>
